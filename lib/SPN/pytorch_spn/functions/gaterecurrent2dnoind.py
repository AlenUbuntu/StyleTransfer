...
from .._ext import gaterecurrent2dnoind as gaterecurrent2dnoind
...
class GateRecurrent2dnoindFunction(Function):
    ...
    def forward(self, X, G1, G2, G3):
        ...
        if not X.is_cuda:
            ...
            output = output.cuda()
            gaterecurrent2dnoind.gaterecurrent2dnoind_forward_cuda(
                self.horizontal, self.reverse, X, G1, G2, G3, output)
    ...
    def backward(self, grad_output):
        ...
        gaterecurrent2dnoind.gaterecurrent2dnoind_backward_cuda(
            self.horizontal, self.reverse, self.output, grad_output, self.X, self.G1, self.G2, self.G3, grad_X, grad_G1, grad_G2, grad_G3)
