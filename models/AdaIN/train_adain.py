...
def train_adain():
    parser = argparse.ArgumentParser(description='PyTorch Style Transfer -- AdaIN')
    ...
    writer = SummaryWriter(log_dir=os.path.join(cfg.OUTPUT_DIR, 'AdaIN'))
    # start training
    for i in iterator:
        ...
        g_t, style_feats, t = model(content_img, style_img, alpha=cfg.MODEL.ALPHA)

        content_loss, style_loss = model.cal_loss(g_t, style_feats, t)

        # compute weighted total loss
        loss = cfg.MODEL.ADAIN.CONTENT_WEIGHT * content_loss + cfg.MODEL.ADAIN.STYLE_WEIGHT * style_loss 
        ...
        if i % 1000 == 0:
            ...
            save_image(all_imgs, os.path.join(cfg.OUTPUT_DIR, 'AdaIN', '{}.jpg'.format(i)), nrow=n)
    
        if i % 10000 == 0:
            torch.save(model.decoder.state_dict(), os.path.join(cfg.OUTPUT_DIR, 'AdaIN', '{}_adain.pth'.format(i)))

    torch.save(model.decoder.state_dict(), os.path.join(cfg.OUTPUT_DIR, 'AdaIN', 'final_adain.pth'))
    writer.close()
